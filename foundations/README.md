...
| <a name="input_nginxaas"></a> [nginxaas](#input\_nginxaas) | A map of Compute Engine region names to F5 NGINXaaS attachment details. | <pre>map(object({<br/>    service_attachment      = optional(string)<br/>    service_account_id      = optional(string)<br/>    tls_certificate_secrets = optional(list(string))<br/>  }))</pre> | `null` | no |
