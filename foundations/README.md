...
| <a name="input_nginxaas"></a> [nginxaas](#input\_nginxaas) | A map of Compute Engine region names to F5 NGINXaaS attachment details. | <pre>object({<br/>    attachments = map(object({<br/>      service_attachment = optional(string)<br/>      service_account_id = optional(string)<br/>    }))<br/>    secrets = optional(set(string))<br/>  })</pre> | `null` | no |
