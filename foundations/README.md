...
| <a name="input_nginxaas"></a> [nginxaas](#input\_nginxaas) | A map of Compute Engine region names to F5 NGINXaaS attachment details. | <pre>map(object({<br/>    service_attachment = optional(string)<br/>    service_account_id = optional(string)<br/>  }))</pre> | `null` | no |
...
| <a name="output_nginxaas"></a> [nginxaas](#output\_nginxaas) | A map of Compute Engine region names to Network Attachments for F5 NGINXaaS integration. |
